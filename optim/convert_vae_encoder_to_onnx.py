...
vae = pipe.vae.to(dtype=torch.float32).eval().to("cuda")  # Use vae directly, ensure fp16
...
sample_input = torch.randn(1, 3, 512, 512).to(torch.float32).to("cuda")
with torch.no_grad():
    torch.onnx.export(
        vae,
        sample_input,
...
sample = torch.randn(1, 3, 512, 512).to(torch.float32).to("cuda")
with torch.no_grad():
