...
vae = pipe.vae.to(dtype=torch.float16).eval().to("cuda")
vae.forward = lambda z: vae.decode(z).sample
sample_input = torch.randn(1, 4, 64, 64).to(torch.float16).to("cuda")
with torch.no_grad():
    torch.onnx.export(
        vae,
        sample_input,
...
sample = torch.randn(1, 4, 64, 64).to(torch.float16).to("cuda")
with torch.no_grad():
